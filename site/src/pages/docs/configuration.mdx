...
| <Cfg p="--body-read-timeout" e="MEMORY_SERVICE_BODY_READ_TIMEOUT" />                       | duration                                  | `30s`           | Maximum time allowed to read ordinary REST request bodies (`0` disables)       |
| <Cfg p="--attachment-body-read-timeout" e="MEMORY_SERVICE_ATTACHMENT_BODY_READ_TIMEOUT" /> | duration                                  | `5m`            | Maximum time allowed to read multipart attachment upload bodies (`0` disables) |
| <Cfg p="--max-page-size" e="MEMORY_SERVICE_MAX_PAGE_SIZE" />                               | positive integer                          | `1000`          | Maximum items accepted by listing endpoints                                    |
| <Cfg p="--temp-dir" e="MEMORY_SERVICE_TEMP_DIR" />                                         | path                                      | system temp dir | Directory for temporary files                                                  |
| <Cfg p="--management-access-log" e="MEMORY_SERVICE_MANAGEMENT_ACCESS_LOG" />               | `true`, `false`                           | `false`         | Enable operational logging for `/health`, `/ready`, `/metrics`                 |
...
| Flag                                                                             | Values         | Default             | Description                                                 |
| -------------------------------------------------------------------------------- | -------------- | ------------------- | ----------------------------------------------------------- |
| <Cfg p="--rate-limit-mode" e="MEMORY_SERVICE_RATE_LIMIT_MODE" />                 | `local`, `off` | `local`             | Enable process-local rate limits or disable them            |
| <Cfg p="--rate-limit-source" e="MEMORY_SERVICE_RATE_LIMIT_SOURCE" />             | rate limit     | `600/1m,burst=100`  | Limit requests from each resolved source IP                 |
| <Cfg p="--rate-limit-identity" e="MEMORY_SERVICE_RATE_LIMIT_IDENTITY" />         | rate limit     | `1200/1m,burst=200` | Limit requests from each authenticated identity             |
| <Cfg p="--rate-limit-auth-failure" e="MEMORY_SERVICE_RATE_LIMIT_AUTH_FAILURE" /> | rate limit     | `30/1m,burst=10`    | Limit authentication failures from each resolved source IP  |
| <Cfg p="--rate-limit-expensive" e="MEMORY_SERVICE_RATE_LIMIT_EXPENSIVE" />       | rate limit     | `60/1m,burst=10`    | Limit expensive operations from each authenticated identity |
| <Cfg p="--rate-limit-stream-open" e="MEMORY_SERVICE_RATE_LIMIT_STREAM_OPEN" />   | rate limit     | `30/1m,burst=5`     | Limit stream openings from each authenticated identity      |

Rate limits use the format `<tokens>/<duration>,burst=<tokens>`. Tokens and burst must be positive integers. Duration must be between `1s` and `1h`. The `off` mode disables all five limits and reports the `rate_limits_off` unsafe configuration metric.
...
A local rate-limit rejection includes `rateLimiter`. Its value is `source`, `identity`, `auth_failure`, `expensive`, or `stream_open`. For example:

```text
WARN http GET /v1/conversations phase=complete requestID=req-123 status=429 duration=4ms result=rate_limited errorCode=rate_limited rateLimiter=identity userID=user-123 clientID=chat-app
```
...
Admin audit records remain separate from operational events. They can contain the configured justification and share the same `requestID`; the canonical operational event never contains the justification. By default, management probes are omitted. Set <Cfg p="--management-access-log" e="MEMORY_SERVICE_MANAGEMENT_ACCESS_LOG" /> to include `/health`, `/ready`, and `/metrics` operations.
...
| <Cfg p="--host" e="MEMORY_SERVICE_HOST" />                                                 | host/IP                | `127.0.0.1`   | HTTP/gRPC bind host; containers should set `0.0.0.0` intentionally                      |
...
| <Cfg p="--allow-non-loopback-plaintext" e="MEMORY_SERVICE_ALLOW_NON_LOOPBACK_PLAINTEXT" /> | `true`, `false`        | `false`       | Explicitly allow plaintext API traffic on a non-loopback TCP bind outside testing mode  |
| <Cfg p="--max-header-bytes" e="MEMORY_SERVICE_MAX_HEADER_BYTES" />                         | bytes                  | `1048576`     | Maximum request header bytes on the main listener                                       |
| <Cfg p="--idle-timeout" e="MEMORY_SERVICE_IDLE_TIMEOUT" />                                 | duration               | `120s`        | HTTP keep-alive idle timeout on the main listener                                       |
| <Cfg p="--trusted-proxy-cidrs" e="MEMORY_SERVICE_TRUSTED_PROXY_CIDRS" />                   | IP/CIDR CSV            | _(unset)_     | Trusted TCP proxy IPs/CIDRs for client-IP resolution; unset trusts no forwarded proxies |
...
| Flag                                                                                         | Values          | Default     | Description                                                                                    |
| -------------------------------------------------------------------------------------------- | --------------- | ----------- | ---------------------------------------------------------------------------------------------- |
| <Cfg p="--management-port" e="MEMORY_SERVICE_MANAGEMENT_PORT" />                             | integer         | _(unset)_   | Dedicated port for health and metrics                                                          |
| <Cfg p="--management-on-main-listener" e="MEMORY_SERVICE_MANAGEMENT_ON_MAIN_LISTENER" />     | `true`, `false` | `false`     | Explicitly acknowledge serving management routes on the main API listener outside testing mode |
| <Cfg p="--management-host" e="MEMORY_SERVICE_MANAGEMENT_HOST" />                             | host/IP         | `127.0.0.1` | Management bind host; containers should set `0.0.0.0` intentionally                            |
| <Cfg p="--management-allow-non-loopback" e="MEMORY_SERVICE_MANAGEMENT_ALLOW_NON_LOOPBACK" /> | `true`, `false` | `false`     | Explicitly allow a dedicated management listener to bind beyond loopback outside testing mode  |
...
| <Cfg p="--management-max-header-bytes" e="MEMORY_SERVICE_MANAGEMENT_MAX_HEADER_BYTES" />     | bytes           | `65536`     | Maximum request header bytes on the management listener                                        |
| <Cfg p="--management-idle-timeout" e="MEMORY_SERVICE_MANAGEMENT_IDLE_TIMEOUT" />             | duration        | `30s`       | HTTP keep-alive idle timeout on the management listener                                        |
...
# MongoDB connection (must target a replica set or mongos; standalone is rejected)

--db-url=mongodb://localhost:27017/memoryservice?replicaSet=rs0`} />
...
| Flag                                                                                                     | Values   | Default                   | Description                                                       |
| -------------------------------------------------------------------------------------------------------- | -------- | ------------------------- | ----------------------------------------------------------------- |
| <Cfg p="--infinispan-url" e="MEMORY_SERVICE_INFINISPAN_URL" />                                           | URL      | `redis://localhost:11222` | Infinispan RESP endpoint (`redis://` plaintext, `rediss://` TLS)  |
| <Cfg p="--infinispan-username" e="MEMORY_SERVICE_INFINISPAN_USERNAME" />                                 | string   | _(none)_                  | Infinispan username (overrides userinfo in URL)                   |
| <Cfg p="--infinispan-password" e="MEMORY_SERVICE_INFINISPAN_PASSWORD" />                                 | string   | _(none)_                  | Infinispan password (overrides userinfo in URL)                   |
| <Cfg p="--infinispan-tls-insecure-skip-verify" e="MEMORY_SERVICE_INFINISPAN_TLS_INSECURE_SKIP_VERIFY" /> | bool     | `false`                   | Skip TLS certificate verification (only applies with `rediss://`) |
| <Cfg p="--infinispan-startup-timeout" e="MEMORY_SERVICE_INFINISPAN_STARTUP_TIMEOUT" />                   | duration | `30s`                     | Timeout waiting for Infinispan RESP endpoint to become ready      |

<ConfigBlock property={`# Enable Infinispan cache (response recording manager will automatically use it)
--cache-kind=infinispan

# Infinispan connection (use rediss:// for TLS)

--infinispan-url=redis://localhost:11222
...
MEMORY_SERVICE_INFINISPAN_URL=redis://localhost:11222
...
- PostgreSQL deployments must run with logical replication enabled because the outbox relay uses `pgoutput`
...
For production attachment deployments, also review the [Security Hardening](/docs/deployment/security/#attachments) guidance on proxying downloads and durable storage.
...
| <Cfg p="--encryption-allow-plain" e="MEMORY_SERVICE_ENCRYPTION_ALLOW_PLAIN" />                   | `true`, `false`     | `false` | Explicitly allow `plain` as the primary provider outside testing; unsafe for production data   |
...
| Flag / Env Var                                           | Values                                               | Default | Description                            |
| -------------------------------------------------------- | ---------------------------------------------------- | ------- | -------------------------------------- |
| <Cfg p="--vector-kind" e="MEMORY_SERVICE_VECTOR_KIND" /> | `none`, `pgvector`, `qdrant`, `sqlite`, `infinispan` | `none`  | Vector store backend                   |
...
The `infinispan` vector backend uses the Infinispan REST API v3 for vector search. Infinispan exposes both RESP and HTTP on the same port (default `11222`), so the cache and vector plugins can share the same server instance.

- Requires Infinispan `16.1` or later for vector search support.
- If `MEMORY_SERVICE_VECTOR_INFINISPAN_URL` is unset, the service derives the REST URL from `MEMORY_SERVICE_INFINISPAN_URL` by translating the scheme: `redis://` → `http://`, `rediss://` → `https://`. Set `MEMORY_SERVICE_INFINISPAN_URL` once to configure both plugins.
- If `MEMORY_SERVICE_VECTOR_INFINISPAN_CACHE_NAME` is unset, memory-service derives a cache name from the embedding model and dimensions.
- `MEMORY_SERVICE_VECTOR_INFINISPAN_USERNAME`, `MEMORY_SERVICE_VECTOR_INFINISPAN_PASSWORD`, and `MEMORY_SERVICE_VECTOR_INFINISPAN_TLS_INSECURE_SKIP_VERIFY` fall back to the shared `MEMORY_SERVICE_INFINISPAN_*` values when unset.

| Flag / Env Var                                                                                                         | Default                                                                | Description                                                        |
| ---------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------ |
| <Cfg p="--vector-infinispan-url" e="MEMORY_SERVICE_VECTOR_INFINISPAN_URL" />                                           | _(derived from `MEMORY_SERVICE_INFINISPAN_URL`)_                       | Infinispan REST endpoint URL (`http://` plaintext, `https://` TLS) |
| <Cfg p="--vector-infinispan-cache-name" e="MEMORY_SERVICE_VECTOR_INFINISPAN_CACHE_NAME" />                             | _(auto)_                                                               | Optional explicit cache name override                              |
| <Cfg p="--vector-infinispan-username" e="MEMORY_SERVICE_VECTOR_INFINISPAN_USERNAME" />                                 | _(falls back to `MEMORY_SERVICE_INFINISPAN_USERNAME`)_                 | Authentication username                                            |
| <Cfg p="--vector-infinispan-password" e="MEMORY_SERVICE_VECTOR_INFINISPAN_PASSWORD" />                                 | _(falls back to `MEMORY_SERVICE_INFINISPAN_PASSWORD`)_                 | Authentication password                                            |
| <Cfg p="--vector-infinispan-auth-type" e="MEMORY_SERVICE_VECTOR_INFINISPAN_AUTH_TYPE" />                               | `digest`                                                               | Auth mechanism: `basic` or `digest`                                |
| <Cfg p="--vector-infinispan-tls-insecure-skip-verify" e="MEMORY_SERVICE_VECTOR_INFINISPAN_TLS_INSECURE_SKIP_VERIFY" /> | _(falls back to `MEMORY_SERVICE_INFINISPAN_TLS_INSECURE_SKIP_VERIFY`)_ | Skip TLS certificate verification (only applies with `https://`)   |

The simplest configuration when using Infinispan for both cache and vector search — set `MEMORY_SERVICE_INFINISPAN_URL` once and both plugins pick it up:

<ConfigBlock
  property={`# Infinispan for cache + vector (shared connection)
--cache-kind=infinispan
--vector-kind=infinispan
--infinispan-url=redis://localhost:11222
--infinispan-username=admin
--infinispan-password=password`}
  env={`# Infinispan for cache + vector (shared connection)
MEMORY_SERVICE_CACHE_KIND=infinispan
...
To override the vector REST endpoint explicitly (e.g. a separate Infinispan cluster for vector search):

<ConfigBlock
  property={`# Infinispan vector store with explicit URL override
--vector-kind=infinispan
--vector-infinispan-url=http://vector-infinispan:11222
--vector-infinispan-username=admin
--vector-infinispan-password=password`}
  env={`# Infinispan vector store with explicit URL override
MEMORY_SERVICE_VECTOR_KIND=infinispan
MEMORY_SERVICE_VECTOR_INFINISPAN_URL=http://vector-infinispan:11222
...
Configure startup policy imports.

| Flag                                                                   | Values       | Default         | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ---------------------------------------------------------------------- | ------------ | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <Cfg p="--policy-import-path" e="MEMORY_SERVICE_POLICY_IMPORT_PATH" /> | CSV of paths | image-dependent | Comma-separated files and directories. The CLI flag can be repeated to append paths. Directories are searched recursively for `.yaml` and `.yml` documents. Only documents with `kind: memory-kind` import immutable database-backed memory-kind versions. Root-level or explicitly listed `authz.rego` and `filter.rego` files replace the built-in global policies. The distributed container image defaults to `/etc/memory-service/policies/`. See [Policy import path configuration](/docs/concepts/memories/#policy-import-path-configuration). |
...
Clients include the API key in requests via the `X-API-Key` header. The API key resolves to the client ID from the environment variable suffix.
...
| Flag                                                                             | Values              | Default  | Description                                                                                 |
| -------------------------------------------------------------------------------- | ------------------- | -------- | ------------------------------------------------------------------------------------------- |
| <Cfg p="--trusted-user-id-clients" e="MEMORY_SERVICE_TRUSTED_USER_ID_CLIENTS" /> | exact client ID CSV | _(none)_ | Clients trusted to assert an effective user on normal user APIs; empty disables the feature |
...
<ConfigBlock
  property={`# Configure the service credential
...
--trusted-user-id-clients=cognition_processor`}
  env={`# Configure the service credential
MEMORY_SERVICE_API_KEYS_COGNITION_PROCESSOR=replace-with-a-secret
...
<ConfigBlock
  property={`--oidc-issuer=https://idp.example.com/realms/memory-service
--oidc-allowed-clients=cognition-processor
--oidc-allowed-audiences=memory-service
--trusted-user-id-clients=cognition-processor`}
  env={`MEMORY_SERVICE_OIDC_ISSUER=https://idp.example.com/realms/memory-service
...
One deployment may trust both API-key and OIDC client IDs:
...
- Invalid or missing credentials remain unauthenticated even when `X-User-ID` is present.
- Admin and system APIs ignore the asserted user.
- Assertion trust grants no admin, auditor, or indexer role. Configure client roles separately with settings such as `MEMORY_SERVICE_ROLES_ADMIN_CLIENTS`.
- When a trusted OIDC client changes the effective user, roles derived from the original token user are dropped; client-derived roles and OIDC scope gates remain in effect.
- `GET /v1/capabilities` and gRPC `SystemService/GetCapabilities` report `auth.user_id_assertion_enabled` without exposing the trusted-client list.
...
| <Cfg p="--oidc-tls-insecure-skip-verify" e="MEMORY_SERVICE_OIDC_TLS_INSECURE_SKIP_VERIFY" /> | `true`, `false` | `false`               | Skip TLS certificate verification for OIDC discovery and JWKS requests; rejected outside testing mode                                                                                                                                      |
| <Cfg p="--oidc-allowed-clients" e="MEMORY_SERVICE_OIDC_ALLOWED_CLIENTS" />                   | CSV             | _(none)_              | Optional OIDC client IDs allowed to call Memory Service; empty allows any client from the configured issuer                                                                                                                                |
| <Cfg p="--oidc-allowed-audiences" e="MEMORY_SERVICE_OIDC_ALLOWED_AUDIENCES" />               | CSV             | _(none)_              | Required OIDC audiences accepted by Memory Service when OIDC is enabled                                                                                                                                                                    |
| <Cfg p="--oidc-user-id-claim" e="MEMORY_SERVICE_OIDC_USER_ID_CLAIM" />                       | JSON Pointer    | `/sub`                | RFC 6901 claim path used as the persistent user identity; authentication fails if the claim is absent, blank, or not a string. Set to `/preferred_username` for Keycloak demo environments. Microsoft Entra deployments should use `/oid`. |
| <Cfg p="--oidc-role-claim" e="MEMORY_SERVICE_OIDC_ROLE_CLAIMS" />                            | JSON Pointer(s) | `/realm_access/roles` | RFC 6901 claim path(s) used for claim-derived roles; env value is a JSON array of pointer strings                                                                                                                                          |
...
Audience-only configuration is valid when the identity provider mints access tokens specifically for Memory Service:

<ConfigBlock
  property={`--oidc-issuer=https://idp.example.com/realms/memory-service
--oidc-allowed-audiences=memory-service-api`}
  env={`MEMORY_SERVICE_OIDC_ISSUER=https://idp.example.com/realms/memory-service
...
| -------------------------------------------------------------------------------- | --------- | ------------------------------------- |
| <Cfg p="--roles-admin-oidc-role" e="MEMORY_SERVICE_ROLES_ADMIN_OIDC_ROLE" />     | `admin`   | OIDC role name that maps to `admin`   |
| <Cfg p="--roles-auditor-oidc-role" e="MEMORY_SERVICE_ROLES_AUDITOR_OIDC_ROLE" /> | `auditor` | OIDC role name that maps to `auditor` |
...
| -------------- | -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `user`         | <Cfg p="--oidc-scopes-user" e="MEMORY_SERVICE_OIDC_SCOPES_USER" />               | One OIDC scope should allow all normal user API reads and writes after normal user authorization passes.            |
| `user_read`    | <Cfg p="--oidc-scopes-user-read" e="MEMORY_SERVICE_OIDC_SCOPES_USER_READ" />     | One OIDC scope should allow all normal user API reads, but writes need a separate scope.                            |
| `user_write`   | <Cfg p="--oidc-scopes-user-write" e="MEMORY_SERVICE_OIDC_SCOPES_USER_WRITE" />   | One OIDC scope should allow all normal user API writes, but reads need a separate scope.                            |
| `admin`        | <Cfg p="--oidc-scopes-admin" e="MEMORY_SERVICE_OIDC_SCOPES_ADMIN" />             | One OIDC scope should allow all admin API reads and writes after normal admin/auditor/indexer authorization passes. |
| `admin_read`   | <Cfg p="--oidc-scopes-admin-read" e="MEMORY_SERVICE_OIDC_SCOPES_ADMIN_READ" />   | One OIDC scope should allow all admin API reads, but writes need a separate scope.                                  |
| `admin_write`  | <Cfg p="--oidc-scopes-admin-write" e="MEMORY_SERVICE_OIDC_SCOPES_ADMIN_WRITE" /> | One OIDC scope should allow all admin API writes, but reads need a separate scope.                                  |

Resource aggregate keys are more granular than `user`/`admin`, but still cover both read and write for a resource family:

| Permission key        | Config property                                                                                  | Use when                                                                                                  |
| --------------------- | ------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------- |
| `conversations`       | <Cfg p="--oidc-scopes-conversations" e="MEMORY_SERVICE_OIDC_SCOPES_CONVERSATIONS" />             | One scope should cover user conversation, entry, fork, child, and response-cancellation reads and writes. |
| `sharing`             | <Cfg p="--oidc-scopes-sharing" e="MEMORY_SERVICE_OIDC_SCOPES_SHARING" />                         | One scope should cover membership and ownership-transfer reads and writes.                                |
| `search`              | <Cfg p="--oidc-scopes-search" e="MEMORY_SERVICE_OIDC_SCOPES_SEARCH" />                           | One scope should cover search/list-unindexed reads and indexing writes.                                   |
| `memories`            | <Cfg p="--oidc-scopes-memories" e="MEMORY_SERVICE_OIDC_SCOPES_MEMORIES" />                       | One scope should cover user memory reads and writes.                                                      |
| `attachments`         | <Cfg p="--oidc-scopes-attachments" e="MEMORY_SERVICE_OIDC_SCOPES_ATTACHMENTS" />                 | One scope should cover user attachment reads and writes.                                                  |
| `recordings`          | <Cfg p="--oidc-scopes-recordings" e="MEMORY_SERVICE_OIDC_SCOPES_RECORDINGS" />                   | One scope should cover gRPC response-recording reads and writes.                                          |
| `admin_conversations` | <Cfg p="--oidc-scopes-admin-conversations" e="MEMORY_SERVICE_OIDC_SCOPES_ADMIN_CONVERSATIONS" /> | One scope should cover admin conversation and entry reads and writes.                                     |
| `admin_memories`      | <Cfg p="--oidc-scopes-admin-memories" e="MEMORY_SERVICE_OIDC_SCOPES_ADMIN_MEMORIES" />           | One scope should cover admin memory, policy, usage, and index APIs.                                       |
| `admin_attachments`   | <Cfg p="--oidc-scopes-admin-attachments" e="MEMORY_SERVICE_OIDC_SCOPES_ADMIN_ATTACHMENTS" />     | One scope should cover admin attachment reads and writes.                                                 |
| `admin_checkpoints`   | <Cfg p="--oidc-scopes-admin-checkpoints" e="MEMORY_SERVICE_OIDC_SCOPES_ADMIN_CHECKPOINTS" />     | One scope should cover admin checkpoint reads and writes.                                                 |

Read/write-specific keys are the most granular option:

| Permission key              | Config property                                                                                              | Use when                                                                                                          |
| --------------------------- | ------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------- |
| `system_read`               | <Cfg p="--oidc-scopes-system-read" e="MEMORY_SERVICE_OIDC_SCOPES_SYSTEM_READ" />                             | Capabilities and authenticated system reads need their own scope. Existing public health endpoints remain public. |
| `conversations_read`        | <Cfg p="--oidc-scopes-conversations-read" e="MEMORY_SERVICE_OIDC_SCOPES_CONVERSATIONS_READ" />               | Conversation, entry, fork, and child reads need a separate scope.                                                 |
| `conversations_write`       | <Cfg p="--oidc-scopes-conversations-write" e="MEMORY_SERVICE_OIDC_SCOPES_CONVERSATIONS_WRITE" />             | Conversation, entry, sync, and response-cancellation writes need a separate scope.                                |
| `sharing_read`              | <Cfg p="--oidc-scopes-sharing-read" e="MEMORY_SERVICE_OIDC_SCOPES_SHARING_READ" />                           | Membership and ownership-transfer reads need a separate scope.                                                    |
| `sharing_write`             | <Cfg p="--oidc-scopes-sharing-write" e="MEMORY_SERVICE_OIDC_SCOPES_SHARING_WRITE" />                         | Membership and ownership-transfer writes need a separate scope.                                                   |
| `search_read`               | <Cfg p="--oidc-scopes-search-read" e="MEMORY_SERVICE_OIDC_SCOPES_SEARCH_READ" />                             | Search and list-unindexed reads need a separate scope.                                                            |
| `search_write`              | <Cfg p="--oidc-scopes-search-write" e="MEMORY_SERVICE_OIDC_SCOPES_SEARCH_WRITE" />                           | Indexing writes need a separate scope.                                                                            |
| `memories_read`             | <Cfg p="--oidc-scopes-memories-read" e="MEMORY_SERVICE_OIDC_SCOPES_MEMORIES_READ" />                         | User memory reads need a separate scope.                                                                          |
| `memories_write`            | <Cfg p="--oidc-scopes-memories-write" e="MEMORY_SERVICE_OIDC_SCOPES_MEMORIES_WRITE" />                       | User memory writes need a separate scope.                                                                         |
| `attachments_read`          | <Cfg p="--oidc-scopes-attachments-read" e="MEMORY_SERVICE_OIDC_SCOPES_ATTACHMENTS_READ" />                   | User attachment reads need a separate scope.                                                                      |
| `attachments_write`         | <Cfg p="--oidc-scopes-attachments-write" e="MEMORY_SERVICE_OIDC_SCOPES_ATTACHMENTS_WRITE" />                 | User attachment writes need a separate scope.                                                                     |
| `events_read`               | <Cfg p="--oidc-scopes-events-read" e="MEMORY_SERVICE_OIDC_SCOPES_EVENTS_READ" />                             | User event streams need their own scope.                                                                          |
| `recordings_read`           | <Cfg p="--oidc-scopes-recordings-read" e="MEMORY_SERVICE_OIDC_SCOPES_RECORDINGS_READ" />                     | gRPC response-recording reads need a separate scope.                                                              |
| `recordings_write`          | <Cfg p="--oidc-scopes-recordings-write" e="MEMORY_SERVICE_OIDC_SCOPES_RECORDINGS_WRITE" />                   | gRPC response-recording writes need a separate scope.                                                             |
| `admin_conversations_read`  | <Cfg p="--oidc-scopes-admin-conversations-read" e="MEMORY_SERVICE_OIDC_SCOPES_ADMIN_CONVERSATIONS_READ" />   | Admin conversation and entry reads need a separate scope.                                                         |
| `admin_conversations_write` | <Cfg p="--oidc-scopes-admin-conversations-write" e="MEMORY_SERVICE_OIDC_SCOPES_ADMIN_CONVERSATIONS_WRITE" /> | Admin conversation writes need a separate scope.                                                                  |
| `admin_memories_read`       | <Cfg p="--oidc-scopes-admin-memories-read" e="MEMORY_SERVICE_OIDC_SCOPES_ADMIN_MEMORIES_READ" />             | Admin memory, policy, usage, and index reads need a separate scope.                                               |
| `admin_memories_write`      | <Cfg p="--oidc-scopes-admin-memories-write" e="MEMORY_SERVICE_OIDC_SCOPES_ADMIN_MEMORIES_WRITE" />           | Admin memory, policy, and indexing writes need a separate scope.                                                  |
| `admin_attachments_read`    | <Cfg p="--oidc-scopes-admin-attachments-read" e="MEMORY_SERVICE_OIDC_SCOPES_ADMIN_ATTACHMENTS_READ" />       | Admin attachment reads need a separate scope.                                                                     |
| `admin_attachments_write`   | <Cfg p="--oidc-scopes-admin-attachments-write" e="MEMORY_SERVICE_OIDC_SCOPES_ADMIN_ATTACHMENTS_WRITE" />     | Admin attachment writes need a separate scope.                                                                    |
| `admin_events_read`         | <Cfg p="--oidc-scopes-admin-events-read" e="MEMORY_SERVICE_OIDC_SCOPES_ADMIN_EVENTS_READ" />                 | Admin event streams need their own scope.                                                                         |
| `admin_checkpoints_read`    | <Cfg p="--oidc-scopes-admin-checkpoints-read" e="MEMORY_SERVICE_OIDC_SCOPES_ADMIN_CHECKPOINTS_READ" />       | Admin checkpoint reads need a separate scope.                                                                     |
| `admin_checkpoints_write`   | <Cfg p="--oidc-scopes-admin-checkpoints-write" e="MEMORY_SERVICE_OIDC_SCOPES_ADMIN_CHECKPOINTS_WRITE" />     | Admin checkpoint writes need a separate scope.                                                                    |
| `admin_stats_read`          | <Cfg p="--oidc-scopes-admin-stats-read" e="MEMORY_SERVICE_OIDC_SCOPES_ADMIN_STATS_READ" />                   | Admin stats APIs need their own read scope.                                                                       |
| `admin_maintenance_write`   | <Cfg p="--oidc-scopes-admin-maintenance-write" e="MEMORY_SERVICE_OIDC_SCOPES_ADMIN_MAINTENANCE_WRITE" />     | Admin eviction and maintenance APIs need their own write scope.                                                   |

<ConfigBlock property={`# Aggregate scope accepted for conversation reads and writes.
...
--oidc-scopes-admin=memory-service:admin`}
  env={`# Aggregate scope accepted for conversation reads and writes.
...
at startup. See [Security Hardening](/docs/deployment/security/#authentication) for the
production stance.
...
| Flag / Env Var                                                                             | Values            | Default              | Description                                                   |
| ------------------------------------------------------------------------------------------ | ----------------- | -------------------- | ------------------------------------------------------------- |
| <Cfg p="--developer-frontend-enabled" e="MEMORY_SERVICE_DEVELOPER_FRONTEND_ENABLED" />     | `true`, `false`   | `false`              | Enable the developer frontend                                 |
| <Cfg p="--developer-frontend-dir" e="MEMORY_SERVICE_DEVELOPER_FRONTEND_DIR" />             | path              | _(none)_             | Directory containing built frontend assets                    |
| <Cfg p="--developer-frontend-client-id" e="MEMORY_SERVICE_DEVELOPER_FRONTEND_CLIENT_ID" /> | string            | `developer-frontend` | Client ID used by the frontend                                |
| <Cfg p="--developer-frontend-auth-mode" e="MEMORY_SERVICE_DEVELOPER_FRONTEND_AUTH_MODE" /> | `oidc`, `api-key` | `oidc`               | Authentication mode                                           |
| <Cfg p="--developer-frontend-api-key" e="MEMORY_SERVICE_DEVELOPER_FRONTEND_API_KEY" />     | string            | _(none)_             | Browser-visible credential used only in `api-key` mode        |
| <Cfg p="--base-url" e="MEMORY_SERVICE_BASE_URL" />                                         | URL               | _(auto)_             | External URL used by runtime configuration and OIDC redirects |
...
<ConfigBlock
  property={`# Enable developer frontend
...
When using the official Docker image, the developer frontend is pre-built. Set `MEMORY_SERVICE_DEVELOPER_FRONTEND_ENABLED=true` to activate it.
