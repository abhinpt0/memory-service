...
# Filter by an exact string metadata value
curl --globoff "http://localhost:8080/v1/admin/conversations?metadata[status]=waiting" \
  -H "Authorization: Bearer <admin-token>"
```

Only one metadata filter is accepted per request. Metadata filter keys may contain alphanumeric characters, underscores, and hyphens; dots are rejected. Matching is exact and string-only, so numeric and boolean metadata values do not match the query value.
...
| `GET /admin/v1/memory-policies` / `PUT /admin/v1/memory-policies`           | admin            | Read or update the memory policy bundle          |
| `GET /admin/v1/memory-index/status` / `POST /admin/v1/memory-index/trigger` | admin            | Inspect or trigger memory vector indexing        |
| `GET /admin/v1/memory-usage` / `GET /admin/v1/memory-usage/top`             | admin            | Inspect memory usage counters                    |
