...
- optional attribution for which logical agent the child conversation belongs to.

Memory Service models that with:

- optional conversation-level `agentId`,
- required conversation-level `clientId` derived from authenticated client context,
...
Each conversation has exactly one `clientId`, and may optionally have one `agentId`:

```text
conversation.clientId = assistant-platform
conversation.agentId  = planner
```

This matters most for the `context` channel. Context is isolated by conversation, and context access is authorized by `conversation.clientId`. If another logical agent needs its own working state, it should use a different conversation.
...
If the child is forked, every branch keeps the child's `startedByConversationId`
and optional `startedByEntryId`. The fork is another branch of the same logical
child, not a second delegated task.
...
The server creates the child conversation and writes that first entry in one operation. Archiving or unarchiving the parent changes only the parent's fork group. If retention eviction later hard-deletes the parent group, the child group remains readable. Its `startedByConversationId` and optional `startedByEntryId` remain as provenance, even though the referenced parent no longer exists.
...
- `ancestry=children` returns every branch whose logical fork tree is a child.
...
This endpoint returns each logical child fork tree once. Its representative is
the original child conversation. Use `ancestry=children&mode=all` to list every
branch, or request `/v1/conversations/{childId}/forks` to navigate that child's
fork tree.
...
5. The researcher agent writes follow-up entries in that child conversation.
...
- each delegated task needs isolated `context` state, or
