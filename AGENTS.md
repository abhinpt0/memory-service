...
- Conversation metadata filter contract: `GET /v1/conversations` uses repeated `metadata=<key><op><value>` query parameters (OpenAPI `form` style with `explode: true`); generated Java REST clients (`ConversationsApi`) use `List<String>` for this parameter; generated gRPC Java stubs expose repeated typed `ConversationMetadataFilter` values (not strings); the hand-written Python (`MemoryServiceProxy`) and TypeScript (`createMemoryServiceProxy`) convenience wrappers accept a list of expression strings and forward them as repeated parameters. Operators are `=` (equal) and `!=` (not equal). Predicates combine with AND. A request may contain at most five predicates. Metadata filter keys may only contain ASCII letters, digits, underscores, and hyphens — dots are rejected. Filters match only scalar string values; missing keys, `null`, numbers, booleans, arrays, and objects do not match either operator. One legacy deep-object equality form (`metadata[key]=value`) remains temporarily compatible but is not advertised in OpenAPI. Mixing the legacy form with the new repeated parameter returns 400. The same key-validation and string-only rules apply to admin list filtering.
...
- Fork navigation: user-facing `/v1/conversations/{id}/forks` and admin `ListForks` return complete `{conversationIds, forkPoints}` navigation snapshots. The separate admin `ListChildConversations` operation provides paginated direct-child summaries.
- Started-by lineage is logical fork-tree metadata: the original conversation in a fork group stores `startedByConversationId` and optional `startedByEntryId`, and reads project them onto every branch. These fields are soft references; archive and unarchive affect only the selected fork group, while eviction explicitly expands through started-child groups and emits each descendant conversation's deletion event. The dedicated `/children` API returns the original conversation once per logical child tree; `ancestry=children&mode=all` returns all branches explicitly.
