...
- Started-by lineage is logical fork-tree metadata: the original conversation in a fork group stores `startedByConversationId` and optional `startedByEntryId`, and reads project them onto every branch. These fields are soft references that may dangle after the parent group is hard-evicted; parent archive, unarchive, and eviction operations do not change child groups. The dedicated `/children` API returns the original conversation once per logical child tree; `ancestry=children&mode=all` returns all branches explicitly.
