...
func (s *PostgresStore) encryptConversationTitle(conversationID string, title string) ([]byte, error) {
	if s.enc == nil {
...
func (s *PostgresStore) decryptConversationTitle(conversationID string, data []byte) (string, error) {
...
func (s *PostgresStore) encryptEntryContent(entryID uuid.UUID, content []byte) ([]byte, error) {
...
func (s *PostgresStore) decryptEntryContent(entryID uuid.UUID, data []byte) ([]byte, error) {
	if s.enc == nil || data == nil {
		return data, nil
	}
...
func (s *PostgresStore) ListForks(ctx context.Context, userID string, conversationID string, clientID *string) (*registrystore.ConversationForkNavigation, error) {
	return s.listForks(ctx, userID, conversationID, registrystore.ForkNavigationVisibility{ClientID: clientID})
}

func (s *PostgresStore) listForks(ctx context.Context, userID string, conversationID string, visibility registrystore.ForkNavigationVisibility) (*registrystore.ConversationForkNavigation, error) {
	conv, found, err := s.lookupConversation(ctx, "id = ?", conversationID)
	if err != nil {
		return nil, err
	}
	...
	if err := db.Where("conversation_group_id = ?", conv.ConversationGroupID).Order("created_at ASC, id ASC").Find(&convs).Error; err != nil {
		return nil, fmt.Errorf("failed to list fork group: %w", err)
	}
	...
	entryIDs := make([]uuid.UUID, 0, len(directRows)+len(ancestry))
	for _, row := range directRows {
	...
	firstEntrySQL := fmt.Sprintf(`SELECT * FROM (
		SELECT e.*, ROW_NUMBER() OVER (PARTITION BY e.conversation_id ORDER BY e.created_at ASC, e.seq ASC NULLS FIRST, e.id ASC) AS fork_row
	...
	entryMap := make(map[uuid.UUID]model.Entry, len(entries)+len(firstEntries))
...
func decryptEntries(s *PostgresStore, entries []model.Entry) error {
	for i := range entries {
		decrypted, err := s.decryptEntryContent(entries[i].ID, entries[i].Content)
		if err != nil {
...
func flattenMemoryContent(s *PostgresStore, entries []model.Entry) ([]any, error) {
	result := make([]any, 0)
	for _, entry := range entries {
